...
describe("Testes a authenticacao do usuario", () => {
    it("Deve falhar devido ao usuario nao existir", async () => {
       const response = await authLocal({body: user}, responseMock(), UserModelMock().userDontExists()  )

       expect(response.status).toBe(404)
    })

    it("Deve falhar na verificacao da senha", async () => {
        const response =  await authLocal({body: user}, responseMock(), UserModelMock().passwordIncorret())

        expect(response.status).toBe(404)
    })

    it("Deve falhar ao logar usuario", async () => {
        const response =  await authLocal({body: user}, responseMock(), UserModelMock().problemLogin())

        expect(response.status).toBe(500)
    })

    it("Deve Logar usuario com sucesso", async () => {
        const response =  await authLocal({body: user}, responseMock(), UserModelMock().loginSuccess())
